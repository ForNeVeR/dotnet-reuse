...
### Added
- `ReuseDirectory.ReadEntries` now supports `REUSE.toml` files, including nested ones and all the `precedence` values (`closest`, `aggregate`, and `override`). If both `REUSE.toml` and `.reuse/dep5` files are present, the method throws an exception, as the REUSE specification prohibits using both.
